...
for i in range(0, n):
    ...
    diameter = np.random.randint(15, 20)
    x = np.random.randint(0, L - diameter)
    y = np.random.randint(0, L - diameter)
    image = Image.new('1', (L, L), 0)
    draw = ImageDraw.Draw(image)
    draw.ellipse((x, y, x+diameter, y+diameter), fill='white', outline='white')
    ...
    image = Image.new('1', (L, L), 0)
    draw = ImageDraw.Draw(image)
    draw.rectangle((x, y, x+diameter, y+diameter), fill='white', outline='white')
...
#     img.save(f'../data/CirclesSquares/X{i + 1}_Y_{Y[i]:.0f}.jpg', 'JPEG')
# exit(0)
...
for i in range(3, len(combinations)):
    ...
    plt.savefig(f"../data/CirclesSquares_{i}.png", dpi=300)
